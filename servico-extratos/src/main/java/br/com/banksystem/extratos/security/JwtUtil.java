...
public class JwtUtil {
    @Value("${jwt.secret}") private String segredo;
    private SecretKey obterChave() { return Keys.hmacShaKeyFor(segredo.getBytes(StandardCharsets.UTF_8)); }
    ...
    public boolean validarToken(String token) {
        try { Jwts.parser().verifyWith(obterChave()).build().parseSignedClaims(token); return true; }
